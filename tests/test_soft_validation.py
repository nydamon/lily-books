...
from lily_books.models import FlowState, ChapterDoc, ParaPair, QAReport
...
def test_qa_text_node_soft_validation():
    ...
    with patch('lily_books.graph.qa_chapter') as mock_qa:
        ...
        with patch('lily_books.graph.save_qa_issues'), \
             patch('lily_books.graph.save_chapter_doc'), \
...
def test_qa_chapter_soft_validation():
    ...
    with patch('lily_books.chains.checker.create_llm_with_fallback') as mock_llm_factory:
        mock_llm = MagicMock()
        ...
        with patch('lily_books.chains.checker.checker_chain', mock_chain), \
             patch('lily_books.chains.checker.create_observability_callback'):
...
def test_qa_text_node_continues_on_failure():
    ...
    with patch('lily_books.graph.qa_chapter', side_effect=mock_qa_side_effect), \
         patch('lily_books.graph.save_qa_issues'), \
         patch('lily_books.graph.save_chapter_doc'), \
         patch('lily_books.graph.save_chapter_failure'), \
         patch('lily_books.graph.append_log_entry'):
