...
from fixtures.sample_chapter import get_sample_chapter_split, get_sample_chapter_doc, get_sample_text
...
def test_rewrite_chapter(mock_llm_factory):
    ...
    with patch('src.lily_books.chains.writer.create_llm_with_fallback', mock_llm_factory):
        ...
        assert result.pairs[0].modern in ["Modernized paragraph 1", "Original paragraph 1"]
...
def test_qa_chapter(mock_llm_factory):
    ...
    # Mock the chain creation by patching the function that creates it
    with patch('src.lily_books.chains.checker.create_llm_with_fallback', mock_llm_factory):
...
def test_structured_outputs():
    ...
    from lily_books.models import WriterOutput, CheckerOutput, ModernizedParagraph
...
def test_token_counting_integration():
    ...
    from lily_books.utils.tokens import count_tokens, calculate_optimal_batch_size
