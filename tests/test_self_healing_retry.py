...
def test_retry_with_llm_enhancement(mock_enhance):
    ...
    mock_enhance.assert_called_once_with(
        "Original prompt", previous_error, 2, "writer"
    )
...
def test_retry_attempt_numbering():
    ...
    for attempt in range(1, 4):
        enhanced = enhance_prompt_on_retry(original_prompt, error_context, attempt, "writer")
...
def test_chain_type_specific_guidance():
    ...
    writer_enhanced = enhance_prompt_on_retry(original_prompt, error_context, 1, "writer")
    assert "paragraphs" in writer_enhanced
    assert "modernization" in writer_enhanced
    
    # Checker-specific guidance
    checker_enhanced = enhance_prompt_on_retry(original_prompt, error_context, 1, "checker")
