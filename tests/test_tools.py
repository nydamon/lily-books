...
def test_tts_fish_audio(mock_run, mock_check_call, mock_session, mock_tts_request, mock_settings):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = tts_fish_audio("This is a test text for TTS generation.", "test_reference_id", wav_path)
        ...
        with patch('lily_books.tools.tts.Session', None):
            with patch('lily_books.tools.tts.TTSRequest', None):
                with pytest.raises(RuntimeError, match="Fish Audio SDK not installed"):
                    tts_fish_audio("This is a test text for error handling.", "invalid_reference", wav_path)
