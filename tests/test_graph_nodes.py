...
from lily_books.graph import qa_text_node, rewrite_node
from lily_books.models import ChapterDoc, ChapterSplit, FlowState, ParaPair, QAReport
from lily_books.storage import save_chapter_doc
...
def test_rewrite_node_with_fallback_models():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        chapter_split = ChapterSplit(
            chapter=1, title="Chapter 1", paragraphs=["Test paragraph"]
        )

        # Mock the LLM factory
        with patch(
            "src.lily_books.chains.writer.create_llm_with_fallback"
        ) as mock_factory:
            mock_llm = MagicMock()
...
def test_qa_text_node_with_fallback_models():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Mock the LLM factory
        with patch(
            "src.lily_books.chains.checker.create_llm_with_fallback"
        ) as mock_factory:
            mock_llm = MagicMock()
            mock_factory.return_value = mock_llm
...
def test_rewrite_node_token_validation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        chapter_split = ChapterSplit(
            chapter=1, title="Chapter 1", paragraphs=long_paragraphs
        )

        # Mock token validation to return invalid
        with patch(
            "src.lily_books.chains.writer.validate_context_window"
        ) as mock_validate:
...
def test_rewrite_node_adaptive_batching():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        chapter_split = ChapterSplit(
            chapter=1, title="Chapter 1", paragraphs=paragraphs
        )

        # Mock batch size calculation
        with patch(
            "src.lily_books.chains.writer.calculate_optimal_batch_size"
        ) as mock_batch:
            ...
            with patch(
                ...
                with patch("src.lily_books.chains.writer.writer_chain") as mock_chain:
                    mock_chain.return_value = {
                        "paragraphs": [
                            {"modern": f"Modernized paragraph {i}"} for i in range(3)
                        ]
                    }
...
def test_observability_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "src.lily_books.chains.writer.create_observability_callback"
        ) as mock_callback:
