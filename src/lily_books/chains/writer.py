...
from ..models import ChapterDoc, ChapterSplit, ParaPair, WriterOutput
from ..utils import llm_factory, tokens
...
from ..utils.debug_logger import debug_async_function, log_step, update_activity
...
GPT5_MINI_FORMAT_INSTRUCTIONS = (
    """Return just the modernized text, one paragraph per line."""
)
...
local_writer_prompt = ChatPromptTemplate.from_messages(
    [("system", WRITER_SYSTEM), ("human", WRITER_USER)]
)
...
def _build_writer_chain(trace_name: str | None = None):
    ...
    chain = (
        {
            ...
            "format_instructions": lambda d: get_format_instructions_for_model(),
        }
        | writer_prompt
        | writer_llm
        | clean_llm_output
...
async def rewrite_chapter_async(
    ch: ChapterSplit, slug: str = None, progress_callback: Callable | None = None
) -> ChapterDoc:
    """Async version of rewrite_chapter with parallel processing and streaming."""
    logger.info(
        f"Starting async rewrite for chapter {ch.chapter} with {len(ch.paragraphs)} paragraphs"
    )
    ...
    logger.info(
        f"Chapter {ch.chapter}: {len(split_paras)} paragraphs, batch_size={batch_size}"
    )

    writer_chain = _build_writer_chain(
        trace_name=f"writer_async_ch{ch.chapter}_{slug}"
        if slug
        else f"writer_async_ch{ch.chapter}"
    )
    ...
    log_step(
        "rewrite_chapter_async.batch_setup",
    ...
    for i, para in enumerate(split_paras):
        ...
        if len(batch) == batch_size or i == len(split_paras) - 1:
            log_step(
                "rewrite_chapter_async.batch_ready",
                batch_number=len(tasks),
                batch_size=len(batch),
            )
            logger.info(
                f"Chapter {ch.chapter}: Creating batch {len(tasks)+1} with {len(batch)} paragraphs"
            )
            ...
            if not is_valid:
                ...
                for single_para, orig_idx in zip(batch, batch_indices):
                    tasks.append(
                        process_single_paragraph_async(
                            single_para,
                            [orig_idx],
                            ch,
                            writer_chain,
...
async def process_batch_async(
    ...
    logger.info(
        f"Starting async batch processing for chapter {ch.chapter}, batch size: {len(batch)}"
    )
    ...
    for attempt in range(1, settings.max_retry_attempts + 1):
        try:
            logger.info(
                f"Async batch processing attempt {attempt} for chapter {ch.chapter}"
            )
            # Run in thread pool to avoid blocking
            loop = asyncio.get_event_loop()
            # Use LangChain chain via OpenRouter
            raw_output = await loop.run_in_executor(
                None, lambda: writer_chain.invoke({"joined": joined}, config=config)
            )
    ...
    for attempt in range(1, settings.max_retry_attempts + 1):
        try:
            ...
            if attempt < settings.max_retry_attempts and should_retry_with_enhancement(
                ...
                logger.warning(
                    f"Single paragraph processing failed (attempt {attempt}), retrying with enhancement: {e}"
                )

                # Enhance the prompt for retry
                enhanced_input = analyze_failure_and_enhance_prompt(
                    {"joined": single_para}, e, attempt, "writer"
                )
                ...
                # Final attempt failed or max attempts reached
                logger.error(
                    f"Single paragraph processing failed after {attempt} attempts: {e}"
                )
...
def process_batch_sync(
    ...
    for attempt in range(1, settings.max_retry_attempts + 1):
        try:
            # Use LangChain chain via OpenRouter
            logger.info(
                f"Processing batch with {len(batch)} paragraphs, attempt {attempt}"
            )
            logger.info("Using LangChain writer chain via OpenRouter")
            raw_output = writer_chain.invoke({"joined": joined}, config=config)
            logger.info(
                f"LangChain writer chain completed, output type: {type(raw_output)}"
            )
            ...
            check_llm_response(
                raw_output, f"writer_chain batch processing (attempt {attempt})"
            )
...
def process_single_paragraph_sync(
    ...
    for attempt in range(1, settings.max_retry_attempts + 1):
        try:
            ...
            check_llm_response(
                raw_output,
                f"writer_chain single paragraph processing (attempt {attempt})",
            )
...
def rewrite_chapter(ch: ChapterSplit, slug: str = None) -> ChapterDoc:
    ...
    writer_chain = _build_writer_chain(
        trace_name=f"writer_sync_ch{ch.chapter}_{slug}"
        if slug
        else f"writer_sync_ch{ch.chapter}"
    )
    ...
    for i, para in enumerate(split_paras):
        ...
        if len(batch) == batch_size or i == len(split_paras) - 1:
            ...
            if not is_valid:
                ...
                for single_para, orig_idx in zip(batch, batch_indices):
                    batch_pairs = process_single_paragraph_sync(
                        single_para, [orig_idx], ch, writer_chain, config, split_paras
                    )
            ...
            # Log token usage
            log_token_usage(
                joined, settings.openai_model, f"rewrite_chapter_{ch.chapter}"
            )
