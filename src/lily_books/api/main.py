...
from ..models import ChapterDoc, ParaPair, BookMetadata
from ..storage import (
    load_state, save_state, load_chapter_doc, save_chapter_doc,
    load_qa_issues, load_book_metadata, save_book_metadata,
    append_log_entry, get_project_paths
)
...
@app.get("/api/projects/{slug}/chapters/{chapter_num}/pairs", response_model=ChapterPairsResponse)
...
async def retry_chapter(slug: str, chapter_num: int) -> Dict[str, str]:
    ...
    append_log_entry(slug, {
        "action": "chapter_retry_requested",
        "chapter": chapter_num
    })
        ...
        if chapter_doc and chapter_doc.pairs:
            ...
            for pair in chapter_doc.pairs:
                if pair.qa:
                    ...
                    if not (pair.qa.modernization_complete and pair.qa.formatting_preserved):
