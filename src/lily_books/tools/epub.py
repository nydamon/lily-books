...
from ..models import BookMetadata, ChapterDoc, ParaPair, PublishingMetadata
...
def build_epub(
    ...
    cover_page = epub.EpubHtml(
        title="Cover", file_name="cover.xhtml", lang=metadata.language
    )
    ...
    if publishing_metadata:
        ...
        copyright_page = epub.EpubHtml(
            title="Copyright", file_name="copyright.xhtml", lang=metadata.language
        )
        ...
        about_page = epub.EpubHtml(
            title="About This Edition", file_name="about.xhtml", lang=metadata.language
        )
    ...
    for chapter_doc in chapters:
        ...
        if not valid_pairs:
            logger.warning(
                f"Skipping chapter with no valid paragraphs: {chapter_doc.title}"
            )
            continue
