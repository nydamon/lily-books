    ...
    try:
        ...
        subprocess.check_call(
            [
                ...
                str(input_wav),
                "-af",
                f"highpass=f=80,lowpass=f=8000,loudnorm=I={target_rms_db}:TP=-3:LRA=11",
                "-c:a",
                "libmp3lame",
                "-b:a",
                "192k",
...
def extract_retail_sample(
    chapter_mp3: Path, start_sec: int, duration_sec: int, output: Path
) -> dict:
...
@tool
def master_audio_tool(
    input_wav: str, output_mp3: str, target_rms_db: float = -20
) -> str:
    """Master audio using ffmpeg ACX-compliant chain.
...
@tool
def extract_retail_sample_tool(
    chapter_mp3: str, start_sec: int, duration_sec: int, output: str
) -> str:
    """Extract retail sample from mastered chapter audio.
    ...
    """
    result = extract_retail_sample(
        Path(chapter_mp3), start_sec, duration_sec, Path(output)
    )
    return json.dumps(result)
