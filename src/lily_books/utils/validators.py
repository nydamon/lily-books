...
from ..models import CheckerOutput, ModernizedParagraph, WriterOutput

logger = logging.getLogger(__name__)


def safe_parse_writer_output(output: Any) -> WriterOutput | None:
    """
    ...
    try:
        if isinstance(output, WriterOutput):
            ...
            try:
                ...
                logger.warning(
                    f"Unexpected string WriterOutput that cannot be parsed: {output[:100]}"
                )
                return None
        else:
...
def safe_parse_checker_output(output: Any) -> CheckerOutput | None:
    """
...
def validate_writer_output(
    output: WriterOutput, expected_count: int
) -> tuple[bool, list[str]]:
...
def validate_paragraph_pair(
    orig: str, modern: str, max_ratio: float = 3.0
) -> tuple[bool, list[str]]:
...
def validate_batch_consistency(
    originals: list[str], moderns: list[str]
) -> tuple[bool, list[str]]:
...
def safe_validate_writer_output(
    output: WriterOutput, expected_count: int
) -> WriterOutput:
    ...
    for idx in range(expected_count):
        if idx == 0:
            paragraphs.append(
                ModernizedParagraph(
                    modern=f"[Validation failed] {fallback_messages[0]}"
                )
            )
        else:
...
def safe_validate_checker_output(output: CheckerOutput) -> CheckerOutput:
    ...
    fallback_score = 50
    score = (
        output.fidelity_score if output.fidelity_score is not None else fallback_score
    )
    try:
        score_int = int(score)
    except Exception:
        ...
        for issue in cleaned["issues"]:
            if isinstance(issue, dict):
                ...
                if (
                    ...
                    and len(issue.get("type", "").strip()) > 0
                    and len(issue.get("description", "").strip()) > 0
                ):
...
def sanity_check_writer_output(output: WriterOutput) -> list[str]:
    ...
    logger.info(
        f"Writer output metrics: {len(output.paragraphs)} paragraphs, "
...
def sanity_check_checker_output(output: CheckerOutput) -> list[str]:
    ...
    # Log metrics for observability (not validation)
    logger.info(
        f"Checker output metrics: fidelity={output.fidelity_score}, "
    ...
    if output.fidelity_score is not None:
        if output.fidelity_score < 0 or output.fidelity_score > 100:
            warnings.append(
                f"Fidelity score outside typical range: {output.fidelity_score}"
            )

    if output.readability_grade is not None:
        if output.readability_grade < 0 or output.readability_grade > 20:
            warnings.append(
                f"Readability grade outside typical range: {output.readability_grade}"
            )
...
def create_retry_prompt_enhancement(
    original_prompt: str, previous_error: str, attempt: int, output_type: str = "writer"
) -> str:
    """
...
def log_llm_decision(context: str, decision: Any, reasoning: str | None = None) -> None:
    """
...
def should_retry_with_enhancement(
    error: Exception, attempt: int, max_attempts: int = None
) -> bool:
    """
