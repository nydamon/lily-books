...
def validate_openrouter_auth() -> Dict[str, Any]:
    ...
    if not api_key:
        return {"service": "OPENROUTER", "status": "failed", "error": "OPENROUTER_API_KEY not set"}
    ...
    try:
        ...
        return {
            ...
            "usage": {
                "openai_tokens": openai_response.usage.total_tokens if openai_response.usage else None,
                "anthropic_tokens": anthropic_response.usage.total_tokens if anthropic_response.usage else None
            }
        }
    except OpenAIAuthError as e:
        return {"service": "OPENROUTER", "status": "failed", "error": f"OpenRouter API error: {e}", "details": "AuthenticationError"}
    except HTTPStatusError as e:
        return {"service": "OPENROUTER", "status": "failed", "error": f"OpenRouter API error: {e}", "details": f"HTTPStatusError: {e.response.status_code} - {e.response.text}"}
    except Exception as e:
        return {"service": "OPENROUTER", "status": "failed", "error": f"OpenRouter API error: {e}", "details": str(e)}
...
def validate_fish_audio_auth() -> Dict[str, Any]:
    ...
    if not api_key:
        return {"service": "FISH_AUDIO", "status": "failed", "error": "FISH_API_KEY not set"}
    ...
    try:
        ...
        return {"service": "FISH_AUDIO", "status": "failed", "error": f"Fish Audio API error: {e}", "details": str(e)}
...
def validate_pipeline_auth() -> bool:
    ...
    logger.info(f"{'✅' if openrouter_result['status'] == 'success' else '❌'} {openrouter_result['service']}:")
    ...
    if 'openai_response' in openrouter_result:
        logger.info(f"   OpenAI Response: {openrouter_result['openai_response'][:50]}...")
    if 'anthropic_response' in openrouter_result:
        logger.info(f"   Anthropic Response: {openrouter_result['anthropic_response'][:50]}...")
    if 'usage' in openrouter_result:
        logger.info(f"   Usage: OpenAI={openrouter_result['usage']['openai_tokens']}, Anthropic={openrouter_result['usage']['anthropic_tokens']}")
    ...
    if settings.enable_audio:
        ...
        logger.info(f"{'✅' if fish_audio_result['status'] == 'success' else '❌'} {fish_audio_result['service']}:")
    ...
    if not all_passed:
        ...
        logger.warning("============================================================")
        logger.warning(f"⚠️  SOME AUTHENTICATION SERVICES FAILED!")
