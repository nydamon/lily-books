...
from .chains.ingest import ChapterizeChain, IngestChain
...
from .tools.audio import extract_retail_sample, get_audio_metrics, master_audio
...
def ingest_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(
        state["slug"],
        {"node": "ingest", "book_id": state["book_id"], "status": "started"},
    )
    ...
    try:
        ...
        append_log_entry(
            state["slug"],
            {"node": "ingest", "status": "completed", "text_length": len(raw_text)},
        )
        ...
        append_log_entry(
            state["slug"], {"node": "ingest", "status": "error", "error": str(e)}
        )
        raise IngestError(
...
def chapterize_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(state["slug"], {"node": "chapterize", "status": "started"})
    ...
    try:
        ...
        if requested_chapters:
            ...
            logger.info(
                f"Filtered chapters: {original_count} -> {len(chapters)} (requested: {requested_chapters})"
            )
        ...
        append_log_entry(
            state["slug"], {"node": "chapterize", "status": "error", "error": str(e)}
        )
...
async def rewrite_node_async(
    state: FlowState, progress_callback: Callable | None = None
) -> FlowState:
    """Async version of rewrite_node with parallel chapter processing."""
    append_log_entry(state["slug"], {"node": "rewrite", "status": "started"})
    ...
    try:
        ...
        logger.info(
            f"rewrite_node_async processing {len(state['chapters'])} chapters: {[ch.chapter for ch in state['chapters']]}"
        )
        ...
        for chapter_split in state["chapters"]:
            ...
            tasks.append(
                rewrite_chapter_async(chapter_split, state["slug"], progress_callback)
            )
        ...
        if tasks:
            log_step(
                "rewrite_node_async.parallel_start",
            ...
            async def rate_limited_chapter(task, chapter_num, index):
                async with semaphore:
                    ...
                    try:
                        log_step(
                            "rewrite_node_async.processing_chapter",
                            chapter=chapter_num,
                        ...
                        log_step(
                            "rewrite_node_async.chapter_completed", chapter=chapter_num
                        )
                        ...
                        log_step(
                            "rewrite_node_async.chapter_timeout",
                            chapter=chapter_num,
                            timeout_seconds=timeout,
                        )
                        logger.error(
                            f"Chapter {chapter_num} timed out after {timeout} seconds"
                        )
                        return TimeoutError(f"Chapter {chapter_num} timed out")
                    except Exception as e:
                        log_step(
                            "rewrite_node_async.chapter_error",
                            chapter=chapter_num,
                            error=str(e),
                        )
                        return e
            ...
            results = await asyncio.gather(
                *[
                    rate_limited_chapter(task, chapter_splits[i].chapter, i)
            ...
            log_step(
                "rewrite_node_async.parallel_completed", total_results=len(results)
            )
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    ...
                    save_chapter_failure(
                        state["slug"], chapter_split.chapter, "rewrite", str(result)
                    )
        ...
        append_log_entry(
            state["slug"], {"node": "rewrite", "status": "error", "error": str(e)}
        )
        raise RewriteError(
            ...
            node="rewrite",
            context={
                "chapter_count": len(state["chapters"]) if state["chapters"] else 0
            },
        )
    ...
    try:
        ...
        for chapter_split in state["chapters"]:
            try:
                ...
                failed_chapters.append(chapter_split.chapter)
                save_chapter_failure(
                    state["slug"], chapter_split.chapter, "rewrite", str(e)
                )
...
async def qa_text_node_async(
    ...
    append_log_entry(state["slug"], {"node": "qa_text", "status": "started"})
    ...
    try:
        ...
        for chapter_doc in state["rewritten"]:
            ...
            if chapter_doc.pairs and all(
                pair.qa is not None for pair in chapter_doc.pairs
            ):
            ...
            tasks.append(
                qa_chapter_async(
                    chapter_doc, slug=state["slug"], progress_callback=progress_callback
                )
            )
        ...
        if tasks:
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    ...
                    error_msg = (
                        f"QA crashed for chapter {chapter_doc.chapter}: {str(result)}"
                    )
                    ...
                    save_chapter_failure(
                        state["slug"], chapter_doc.chapter, "qa_text", error_msg
                    )
        ...
        if failed_chapters:
            append_log_entry(
                state["slug"],
                {
                    ...
                    "failed_chapters": failed_chapters,
                    "passed_chapters": len(chapter_docs)
                    - len(failed_chapters)
        ...
        append_log_entry(
            state["slug"], {"node": "qa_text", "status": "error", "error": str(e)}
        )
        raise QAError(
            ...
            node="qa_text",
            context={
                "chapter_count": len(state["rewritten"]) if state["rewritten"] else 0
            },
        )
    ...
    try:
        ...
        for chapter_doc in state["rewritten"]:
            try:
                ...
                passed, issues, updated_doc = qa_chapter(
                    chapter_doc, slug=state["slug"]
                )
                ...
                failed_chapters.append(chapter_doc.chapter)
                save_chapter_failure(
                    state["slug"], chapter_doc.chapter, "qa_text", str(e)
                )
        ...
        if failed_chapters:
            append_log_entry(
                state["slug"],
                {
                    ...
                    "status": "completed_with_failures",
                    "failed_chapters": failed_chapters,
                    "passed_chapters": len(state["rewritten"])
                    - len(failed_chapters)
                    + len(skipped_chapters),
                },
            )
...
def remediate_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(state["slug"], {"node": "remediate", "status": "started"})
    ...
    try:
        ...
        if not failed_chapters:
            ...
            append_log_entry(
                state["slug"],
                {"node": "remediate", "status": "completed", "remediated_count": 0},
            )
            return {**state, "qa_text_ok": True}

        logger.info(
            f"Remediating {len(failed_chapters)} failed chapters: {failed_chapters}"
        )
        ...
        for chapter_num in failed_chapters:
            try:
                # Find chapter data
                chapter_data = next(
                    (ch for ch in chapters_data if ch["chapter"] == chapter_num), None
                )
                if not chapter_data:
                ...
                if passed:
                    ...
                    logger.warning(
                        f"Chapter {chapter_num} still failing after remediation"
                    )
                    still_failing.append(chapter_num)
        ...
        append_log_entry(
            state["slug"], {"node": "remediate", "status": "error", "error": str(e)}
        )
        raise QAError(
            f"Remediate failed: {str(e)}", slug=state["slug"], node="remediate"
        )
...
def metadata_node(state: FlowState) -> FlowState:
    ...
    try:
        ...
        original_title = gutenberg_data.get(
            "title", state["slug"].replace("-", " ").title()
        )
        authors = gutenberg_data.get("authors", [])
        original_author = authors[0]["name"] if authors else "Unknown"

        logger.info(
            f"Fetched Gutenberg metadata: '{original_title}' by {original_author}"
        )
        ...
        append_log_entry(
            state["slug"], {"node": "metadata", "status": "error", "error": str(e)}
        )
...
def cover_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(state["slug"], {"node": "cover", "status": "started"})
    ...
    try:
        ...
        cover_design = generate_cover(metadata=pub_metadata, slug=state["slug"])
        ...
        append_log_entry(
            state["slug"], {"node": "cover", "status": "error", "error": str(e)}
        )
        raise CoverError(
            f"Cover generation failed: {e}", slug=state["slug"], node="cover"
        )


def epub_node(state: FlowState) -> FlowState:
    """Build EPUB from modernized chapters."""
    append_log_entry(state["slug"], {"node": "epub", "status": "started"})
    ...
    try:
        ...
        append_log_entry(
            state["slug"], {"node": "epub", "status": "error", "error": str(e)}
        )
        raise EPUBError(
...
def tts_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(state["slug"], {"node": "tts", "status": "started"})
    ...
    try:
        ...
        append_log_entry(
            state["slug"],
            {"node": "tts", "status": "completed", "total_chapters": len(audio_files)},
        )
        ...
        append_log_entry(
            state["slug"], {"node": "tts", "status": "error", "error": str(e)}
        )
        raise TTSError(
            f"TTS failed: {str(e)}",
            slug=state["slug"],
...
def master_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(state["slug"], {"node": "master", "status": "started"})
    ...
    try:
        ...
        append_log_entry(
            state["slug"], {"node": "master", "status": "error", "error": str(e)}
        )
        raise MasterError(
            ...
            node="master",
            context={
                "audio_file_count": len(state["audio_files"])
...
def qa_audio_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(state["slug"], {"node": "qa_audio", "status": "started"})
    ...
    try:
        ...
        append_log_entry(
            state["slug"],
            {"node": "qa_audio", "status": "completed", "all_passed": all_passed},
        )
        ...
        append_log_entry(
            state["slug"], {"node": "qa_audio", "status": "error", "error": str(e)}
        )
        raise QAError(
            ...
            node="qa_audio",
            context={
                "mastered_file_count": len(state["mastered_files"])
...
def package_node(state: FlowState) -> FlowState:
    ...
    append_log_entry(state["slug"], {"node": "package", "status": "started"})
    ...
    try:
        ...
        publish_data = {
            "title": f"{state['slug'].title()} (Modernized Student Edition)",
            "ebook_path": str(state.get("epub_path"))
            ...
            "completed_at": str(
                append_log_entry(
                    state["slug"], {"node": "package", "status": "completed"}
                )
            ),
        }
        ...
        append_log_entry(
            state["slug"], {"node": "package", "status": "error", "error": str(e)}
        )
        raise PackageError(
...
def build_graph() -> StateGraph:
    ...
    if config.enable_qa_review:
        ...
        graph.add_conditional_edges(
            "qa_text",
            should_remediate,
            {"remediate": "remediate", "metadata": "metadata"},
        )
